...
## 6.4 「上限 1/1 に上げる → 1/2 になった」の切り分け

実機報告：**表示**「上限 1/4 では 1.4段 足りません。上限を 1/1 に上げる」／
**押した結果** 1/2 になり警告が消えた。

### 数値を追った結果：**文言と `action` は食い違っていない**

`shortStops = max(0, ceiling − over)` なので、報告の値から `over` が逆算できる。

```
ceiling = 2（1/4）、shortStops = 1.4  →  over = 2 − 1.4 = 0.6
to           = floor(0.6) = 0  →  上限 1/1
action.to    = 0               →  ボタンも上限 1/1（同じ変数）
powerStops   = max(0, round(0.6)) = 1  →  **発光量 1/2**
shortStops後 = max(0, 0 − 0.6) = 0     →  警告は消える
```

見えた 1/2 は**上限ではなく発光量**（結果パネルの「発光」）。`label` と `data-to` は
同じ `action.to` から作っているので、値がずれる経路が構造上ない。

### `floor` は正しい（`round` にすると警告が消えない）

上限は「これより強い側は選ばない」線なので、`over` を上回る上限を残すと
`shortStops = 上限 − over` が正のまま残る。

| 候補 | shortStops | 結果 |
| --- | --- | --- |
| 上限 1（1/2） | `1 − 0.6 = 0.4` | **0.4段 足りないまま**（1/3段以上なので警告が残る） |
| 上限 0（1/1） | `max(0, 0 − 0.6) = 0` | 消える |

ご指摘の「1段（1/2）が正しい」では届きません。**`floor(over)` が必要量を満たす最も弱い線**です。

### それでも直したこと：**文言が誤解を招いていた**

「（この設定で撮れます）」だけでは、上限と発光量のどちらの話か読み取れない。両方を出す。

| | 変更前 | 変更後 |
| --- | --- | --- |
| 文言 | 上限を 1/1 に上げる**（この設定で撮れます）** | 上限を 1/1 に上げる**（発光量 1/2 で撮れます）** |
| `action` | `{kind, to, label}` | `{kind, to, resultPowerStops, label}` |

`resultPowerStops` は `resolvePowerWithCeiling` と同じ式（`max(to, min(minPower, round(over)))`）。
マニュアル §10 に「上限 1/1 と発光量 1/2 は矛盾していない」の節を新設した。

**テスト W7** が、上限と発光量が1段ずれる組み合わせ（`over = 0.6`、距離 `2.5/2^0.3` = 2.03m）で
① 文言のラベル＝`action.to` ② 文言の発光量＝`action.resultPowerStops`
③ 実際に `to` を適用したら警告が消え発光量が予告どおり ④ そもそも「ずれる例」を踏んでいること
——の4点を検証する。W6 は上限＝発光量になる側（`over = 1.47`）を見ているので、両側を押さえた。
...
- `compute.js` が `isoFloorWarning` を **import していたが呼んでいなかった**（未使用 import）。
  ...
  **関数自体は残している**（テスト #5b・#33 が直接呼ぶ／将来 ISO 優先の意図を足すときに使う）。
  ...
  これは「アプリから呼ばれない関数をテストしている」状態なので、
  **`tests.html` の #5b 直前にその旨と理由をコメントで書いた。** 消さずに残す前提
  （仕様 §9 が定義する警告であり計算は正しい／使っていないから壊れても気づかない状態を防ぐ）と、
  compute に ISO を下げる経路を足したときは呼び出し側の検証に格上げすること、を明記している。
